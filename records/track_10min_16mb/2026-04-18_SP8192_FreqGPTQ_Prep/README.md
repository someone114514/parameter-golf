# Prep: SP8192 + 3-Layer Recurrence + Parallel Residuals + QK-Gain 5.25 + Legal TTT + FreqGPTQ

This directory is a preparation fork of merged PR #1532 with `FreqGPTQ` added only to the GPTQ calibration path. It has not been rerun yet. The metrics below are inherited from the base record for reference only.

**base val_bpb = 1.0810** (3-seed mean, std 0.0002) | **~15.99 MB** | 8xH100 SXM
...
SEED=42 QK_GAIN_INIT=5.25 TTT_ENABLED=1 TTT_LR=0.005 TTT_EPOCHS=3 FREQ_GPTQ_ENABLED=1 \
