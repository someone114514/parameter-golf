...
class Hyperparameters:
    ...
    split_lr_enabled = bool(int(os.environ.get("SPLIT_LR_ENABLED", "0")))
...
def main() -> None:
    ...
    if args.smoke_test:
        ...
        print(f"smoke_test:flash_attn_available={flash_attn_3_func is not None}")
        smoke_model = GPT(
            vocab_size=128,
            num_layers=2,
            model_dim=128,
        ...
        print(f"smoke_test:model_params={sum(p.numel() for p in smoke_model.parameters())}")
        if torch.cuda.is_available() and flash_attn_3_func is not None:
            device = torch.device("cuda")
            smoke_model = smoke_model.to(device).bfloat16()
            x = torch.randint(0, 128, (2, 32), device=device)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = smoke_model.forward_logits(x)
            print(f"smoke_test:forward_ok logits_shape={tuple(logits.shape)}")
        else:
    ...
    if flash_attn_3_func is None:
        raise RuntimeError("flash_attn_interface is required for full train/eval runs")
    ...
    if args.online_ngram_enabled:
        ...
        online_loss, online_bpb, online_meta = eval_val_sliding_online_best_agree(
            ...
            stride=args.eval_stride,
            batch_seqs=args.online_ngram_batch_seqs,
            eval_seq_len=sw_seq_len,
            log0=log0,
        )
        torch.cuda.synchronize()
        log0(
            ...
            f"gain_bpb:{online_meta['gain_bpb']:.8f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_online):.0f}ms"
        )
