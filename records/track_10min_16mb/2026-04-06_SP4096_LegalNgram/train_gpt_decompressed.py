...
_SMOKE_TEST = os.environ.get("SMOKE_TEST", "0") == "1"
try:
...
class Hyperparameters():
    ...
    eval_mode = os.environ.get(
        "EVAL_MODE",
        "slot" if os.environ.get("SLOT_ENABLED", "0") == "1"
        else ("ngram" if os.environ.get("ONLINE_NGRAM_ENABLED", "0") == "1" else "sliding"),
    )
    slot_enabled = bool(int(os.environ.get("SLOT_ENABLED", "1" if eval_mode == "slot" else "0")))
    online_ngram_enabled = bool(int(os.environ.get("ONLINE_NGRAM_ENABLED", "1" if eval_mode == "ngram" else "0")))
...
def _submission_code_bytes() -> int:
    paths = [
        Path(__file__),
        Path(__file__).with_name("online_best_agree_eval.py"),
        Path(__file__).with_name("online_ngram_state.c"),
    ]
...
def serialize(h: Hyperparameters, base_model: torch.nn.Module, code: str) -> int:
    model_bytes = None
    code_bytes = _submission_code_bytes()
...
def timed_eval(label: str, fn, *args, **kwargs) -> tuple[float, float]:
    ...
    if isinstance(result, tuple) and len(result) == 3:
        val_loss, val_bpb, extras = result
        extra_bits = []
        gain_bpb = extras.get("gain_bpb") if isinstance(extras, dict) else None
        if gain_bpb is not None:
            extra_bits.append(f"gain_bpb:{gain_bpb:.8f}")
        extra_suffix = (" " + " ".join(extra_bits)) if extra_bits else ""
    else:
        val_loss, val_bpb = result
        extra_suffix = ""
    log(f"{label} val_loss:{val_loss:.8f} val_bpb:{val_bpb:.8f}{extra_suffix} eval_time:{elapsed_ms:.0f}ms")
    return val_loss, val_bpb


def run_smoke_test(h: Hyperparameters, device: torch.device):
    log("smoke_test:start")
    log(f"smoke_test:flash_attn_available={1 if flash_attn_3_func is not None else 0}")
    smoke_h = copy.copy(h)
    smoke_h.vocab_size = 128
    smoke_h.num_layers = 2
    smoke_h.xsa_last_n = 2
    smoke_h.num_kv_heads = 1
    smoke_h.model_dim = 64
    smoke_h.embedding_dim = 64
    smoke_h.num_heads = 2
    smoke_h.mlp_mult = 2.0
    smoke_h.ve_dim = 32
    smoke_h.ve_layers = "0,1"
    smoke_h.recur_layers = ""
    smoke_h.parallel_start_layer = 99
    smoke_h.eval_seq_len = 32
    smoke_h.train_seq_len = 32
    model = GPT(smoke_h).to(device)
    log(f"smoke_test:model_params={sum(p.numel() for p in model.parameters())}")
    if flash_attn_3_func is None:
        log("smoke_test:forward_skipped model_init_ok")
        if _FLASH_ATTN_IMPORT_ERROR is not None:
            log(f"smoke_test:flash_attn_import_error={type(_FLASH_ATTN_IMPORT_ERROR).__name__}")
        log("smoke_test:ok")
        return
    x = torch.randint(0, smoke_h.vocab_size, (1, smoke_h.eval_seq_len), device=device, dtype=torch.long)
    with torch.no_grad():
        logits = model(x, x, return_logits=True)
    log(f"smoke_test:forward_ok logits_shape={tuple(logits.shape)}")
    log("smoke_test:ok")
...
def run_evals(
    ...
    if h.eval_mode == "slot" and h.slot_enabled and h.eval_stride > 0:
        torch.cuda.synchronize()
    ...
    elif h.eval_mode == "ngram" and h.online_ngram_enabled and h.eval_stride > 0:
        timed_eval(
            "online_best_agree",
            eval_val_sliding_online_best_agree,
            args=h,
            ...
            stride=h.eval_stride,
            batch_seqs=h.online_ngram_batch_seqs,
...
def main():
    ...
    if _SMOKE_TEST and not torch.cuda.is_available():
        h = Hyperparameters()
        set_logging_hparams(h)
        os.makedirs("logs", exist_ok=True)
        run_smoke_test(h, torch.device("cpu"))
        return
    ...
    h = Hyperparameters()
    if h.eval_mode not in {"sliding", "slot", "ngram"}:
        raise ValueError(f"Unknown EVAL_MODE={h.eval_mode}")
    set_logging_hparams(h)
    if h.is_main_process:
        os.makedirs("logs", exist_ok=True)
