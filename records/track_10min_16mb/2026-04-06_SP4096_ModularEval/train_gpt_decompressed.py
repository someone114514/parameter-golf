...
class Hyperparameters():
    ...
    throughput_mode = os.environ.get("THROUGHPUT_MODE", "off")
    compile_enabled = bool(int(os.environ.get("COMPILE_ENABLED", "1" if throughput_mode in {"compile", "combo"} else "0")))

    # Eval orchestration
    eval_mode = os.environ.get("EVAL_MODE", "slot")
    slot_enabled = bool(int(os.environ.get("SLOT_ENABLED", "1" if eval_mode == "slot" else "0")))
    ...
    slot_schedule = os.environ.get("SLOT_SCHEDULE", "constant")
    slot_early_stop = bool(int(os.environ.get("SLOT_EARLY_STOP", "0")))
    slot_param_mode = os.environ.get("SLOT_PARAM_MODE", "baseline")
    slot_target_layers = os.environ.get("SLOT_TARGET_LAYERS", "9,10")
    slot_adam_beta1 = float(os.environ.get("SLOT_ADAM_BETA1", 0.9))
    slot_adam_beta2 = float(os.environ.get("SLOT_ADAM_BETA2", 0.95))
    slot_adam_eps = float(os.environ.get("SLOT_ADAM_EPS", 1e-8))
...
def maybe_compile(obj, h: Hyperparameters):
    if not h.compile_enabled:
        return obj
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        if flash_attn_3_func is None:
            raise RuntimeError("flash_attn_interface is unavailable; install a matching CUDA runtime for training/eval")
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        self.slot_target_layers = parse_layer_list(h.slot_target_layers)
    ...
    def _slot_layer_index(self, phys_idx: int) -> int:
        try:
            return self.slot_target_layers.index(phys_idx)
        except ValueError:
            return -1
    ...
    def forward_logits(
        ...
        slot_per_layer: Tensor | None = None,
        slot_split_branch: tuple[Tensor, Tensor] | None = None,
    ) -> Tensor:
        ...
        for vi in range(num_dec):
            ...
            if is_parallel_mode:
                ...
                slot_idx = self._slot_layer_index(phys_idx)
                if slot_idx >= 0:
                    if slot_per_layer is not None:
                        lane0 = lane0 + slot_per_layer[slot_idx][None, None, :].to(dtype=lane0.dtype)
                    if slot_split_branch is not None:
                        attn_delta, mlp_delta = slot_split_branch
                        lane0 = lane0 + attn_delta[slot_idx][None, None, :].to(dtype=lane0.dtype)
                        lane1 = lane1 + mlp_delta[slot_idx][None, None, :].to(dtype=lane1.dtype)
            else:
                ve = self._get_ve(phys_idx, input_ids, ve_cache)
                x = self.blocks[phys_idx](x, x0, v_embed=ve)
                slot_idx = self._slot_layer_index(phys_idx)
                if slot_idx >= 0 and slot_per_layer is not None:
                    x = x + slot_per_layer[slot_idx][None, None, :].to(dtype=x.dtype)
        ...
        if slot_delta is not None:
            x = x + slot_delta[None, None, :].to(dtype=x.dtype)
...
def eval_val_sliding(
    ...
    base_model.eval()
    logits_fn = maybe_compile(base_model.forward_logits, h)
...
# ----------------------------------------
# Causal SLOT (context-only delta optimization)
# ----------------------------------------

def _make_slot_params(h: Hyperparameters, model: GPT, device: torch.device):
    dim = model.blocks[-1].attn_scale.numel()
    n_layers = len(model.slot_target_layers)
    mode = h.slot_param_mode
    if mode == "baseline":
        return {"slot_delta": nn.Parameter(torch.zeros(dim, device=device, dtype=torch.float32))}
    if mode == "per_layer":
        return {"slot_per_layer": nn.Parameter(torch.zeros(n_layers, dim, device=device, dtype=torch.float32))}
    if mode == "split_branch":
        return {
            "slot_split_attn": nn.Parameter(torch.zeros(n_layers, dim, device=device, dtype=torch.float32)),
            "slot_split_mlp": nn.Parameter(torch.zeros(n_layers, dim, device=device, dtype=torch.float32)),
        }
    raise ValueError(f"Unknown SLOT_PARAM_MODE={mode}")


def _slot_forward_logits(model: GPT, x: Tensor, slot_params: dict[str, Tensor]) -> Tensor:
    if "slot_delta" in slot_params:
        return model.forward_logits(x, slot_delta=slot_params["slot_delta"])
    if "slot_per_layer" in slot_params:
        return model.forward_logits(x, slot_per_layer=slot_params["slot_per_layer"])
    if "slot_split_attn" in slot_params:
        return model.forward_logits(
            x,
            slot_split_branch=(slot_params["slot_split_attn"], slot_params["slot_split_mlp"]),
        )
    raise ValueError("Unsupported SLOT params")
...
def eval_val_slot(
    ...
    log_fn(
        f"slot_sliding:start windows={len(window_starts)} stride={stride} "
        f"slot_lr={h.slot_lr} slot_steps={h.slot_steps} "
        f"slot_mode={h.slot_param_mode} target_layers={base_model.slot_target_layers}"
    )
    ...
    base_model.eval()
    start_bi = 0
    if my_windows and my_windows[0] == 0:
        first_ws = my_windows[0]
        end = min(first_ws + seq_len, total_tokens)
        wlen = end - first_ws
        chunk = val_data.val_tokens[first_ws:end + 1].to(dtype=torch.int64, device=device)
        x_first = chunk[:-1].reshape(1, seq_len)
        y_first = chunk[1:].reshape(1, seq_len)
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            logits = base_model.forward_logits(x_first)
        nll = F.cross_entropy(
            logits.reshape(-1, logits.size(-1)).float(),
            y_first.reshape(-1),
            reduction="none",
        ).reshape(1, seq_len)
        scored_nll = nll[0, :wlen].to(torch.float64)
        loss_sum += scored_nll.sum()
        token_count += float(wlen)
        tgt, prev = y_first[0, :wlen], x_first[0, :wlen]
        tb = val_data.base_bytes_lut[tgt].to(torch.float64)
        tb += (val_data.has_leading_space_lut[tgt] & ~val_data.is_boundary_token_lut[prev]).to(torch.float64)
        byte_count += tb.sum()
        start_bi = 1

    for bi in range(start_bi, len(my_windows), batch_seqs):
        batch_ws = my_windows[bi:bi + batch_seqs]
        bsz = len(batch_ws)
        x_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
        y_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
        ...
        slot_params = _make_slot_params(h, base_model, device)
        opt = torch.optim.AdamW(
            list(slot_params.values()),
            lr=h.slot_lr,
            betas=(h.slot_adam_beta1, h.slot_adam_beta2),
            eps=h.slot_adam_eps,
            weight_decay=h.slot_wd,
            fused=True,
        )

        if h.slot_steps > 0:
            prev_probe = None
            for step in range(h.slot_steps):
                if h.slot_schedule == "cosine":
                    frac = step / max(h.slot_steps - 1, 1)
                    lr = h.slot_lr * 0.5 * (1.0 + math.cos(math.pi * frac))
                    for pg in opt.param_groups:
                        pg["lr"] = lr
                opt.zero_grad(set_to_none=True)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    logits = _slot_forward_logits(base_model, x_batch, slot_params)
                tok_loss = F.cross_entropy(
                    logits.reshape(-1, logits.size(-1)).float(),
                    y_batch.reshape(-1),
                    reduction="none",
                ).reshape(bsz, seq_len)
                context_mask = torch.zeros_like(tok_loss, dtype=torch.float32)
                for i, wlen in enumerate(wlens):
                    cend = max(wlen - stride, 0)
                    if cend > 0:
                        context_mask[i, :cend] = 1.0
                denom = context_mask.sum().clamp_min(1.0)
                loss = (tok_loss * context_mask).sum() / denom
                loss.backward()
                if world_size > 1:
                    for p in slot_params.values():
                        if p.grad is not None:
                            dist.all_reduce(p.grad, op=dist.ReduceOp.AVG)
                opt.step()
                if h.slot_early_stop:
                    probe = float(loss.detach().item())
                    if prev_probe is not None and probe >= prev_probe:
                        break
                    prev_probe = probe
        ...
        if rank == 0 and (bi == 0 or (bi // batch_seqs) % 10 == 0 or bi + batch_seqs >= len(my_windows)):
            rl = loss_sum.item() / max(token_count.item(), 1)
            rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1)) if token_count.item() > 0 else 0.0
            elapsed = time.perf_counter() - t0
            log_fn(f"  slot_batch [{min(bi + bsz, len(my_windows))}/{len(my_windows)}] bpb={rbpb:.6f} time={elapsed:.1f}s")
    ...
    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    log_fn(f"slot_sliding:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} elapsed={time.perf_counter() - t0:.1f}s")
    return val_loss, val_bpb
...
def run_evals(
    ...
    compiled_model = maybe_compile(eval_model, h)
    ...
    if h.slot_enabled:
        ...
        slot_model.load_state_dict(eval_model.state_dict(), strict=True)
        if hasattr(slot_model, 'set_recurrence_active'):
            slot_model.set_recurrence_active(True)
        timed_eval("final_causal_slot", eval_val_slot, h, slot_model, device, val_data, log_fn=log)
...
def train_model(h: Hyperparameters, device: torch.device, val_data: ValidationData) -> None:
    ...
    compiled_model = maybe_compile(base_model, h)
    if h.distributed:
...
def run_smoke_test() -> None:
    ...
    os.environ.setdefault("VOCAB_SIZE", "128")
    ...
    os.environ.setdefault("VE_LAYERS", "1")
    os.environ.setdefault("RECUR_LAYERS", "0")
    os.environ.setdefault("PARALLEL_START_LAYER", "99")
    os.environ.setdefault("SLOT_TARGET_LAYERS", "1")
    h = Hyperparameters()
    device = torch.device("cpu")
    model = GPT(h).to(device)
    nparams = sum(p.numel() for p in model.parameters())
    print(f"smoke_test:model_params={nparams}")
    print("smoke_test:forward_skipped model_init_ok")
    print("smoke_test:ok")


def main():
    if bool(int(os.environ.get("SMOKE_TEST", "0"))):
        run_smoke_test()
        return
