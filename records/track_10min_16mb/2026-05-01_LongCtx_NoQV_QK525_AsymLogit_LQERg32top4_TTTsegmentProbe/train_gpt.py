...
def _ttt_segment_id(batch_num):
    for i, (lo, hi, _) in enumerate(TTT_SEGMENT_BUCKETS):
        if lo <= batch_num <= hi:
            return i
    return len(TTT_SEGMENT_BUCKETS) - 1


def _ttt_tail_update_mult(h, global_ttt_done, batch_num, doc_lens):
    policy = h.ttt_tail_policy
    if policy in ("", "none", "off"):
        return 1.0
    if policy in ("no_update", "no_ttt"):
        return 0.0
    if not global_ttt_done:
        return 1.0
    max_doc_len = max(doc_lens) if doc_lens else 0
    if policy == "batch300_150":
        if batch_num <= 150:
            return 0.0
        if batch_num <= 300:
            return 0.25
        return 1.0
    if policy == "batch300_off":
        return 0.0 if batch_num <= 300 else 1.0
    if policy == "batch200_off":
        return 0.0 if batch_num <= 200 else 1.0
    if policy == "doc512_256":
        if max_doc_len <= 256:
            return 0.0
        if max_doc_len <= 512:
            return 0.25
        return 1.0
    if policy == "doc384_192":
        if max_doc_len <= 192:
            return 0.0
        if max_doc_len <= 384:
            return 0.25
        return 1.0
    raise ValueError(f"Unsupported TTT_TAIL_POLICY={policy!r}")
...
def eval_val_ttt_phased(h, base_model, device, val_data, forward_ttt_train):
    ...
    if h.ttt_segment_log:
        seg_loss = torch.zeros(len(TTT_SEGMENT_BUCKETS), device=device, dtype=torch.float64)
        seg_bytes = torch.zeros_like(seg_loss)
        seg_tokens = torch.zeros_like(seg_loss)
        seg_batches = torch.zeros_like(seg_loss)
    else:
        seg_loss = seg_bytes = seg_tokens = seg_batches = None
    t_start = time.perf_counter()
    ...
    try:
      while True:
        ...
        update_mult = _ttt_tail_update_mult(h, global_ttt_done, batch_num, doc_lens)
        bsz = len(batch)
        ...
        for group in cur_opt.param_groups:
            group["lr"] = h.ttt_lora_lr * h.ttt_local_lr_mult * update_mult
        ...
        for ci in range(max_nc):
            ...
            if needs_train and update_mult > 0.0:
        ...
        if h.ttt_segment_log:
            ...
            if dt > 0 and db > 0:
                sid = _ttt_segment_id(batch_num)
                seg_loss[sid] += cur_loss_val - prev_loss
                seg_bytes[sid] += db
                seg_tokens[sid] += dt
                seg_batches[sid] += 1
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        if h.ttt_segment_log:
            dist.all_reduce(seg_loss, op=dist.ReduceOp.SUM)
            dist.all_reduce(seg_bytes, op=dist.ReduceOp.SUM)
            dist.all_reduce(seg_tokens, op=dist.ReduceOp.SUM)
            dist.all_reduce(seg_batches, op=dist.ReduceOp.SUM)
    if h.ttt_segment_log and h.rank == 0:
        for i, (_, _, name) in enumerate(TTT_SEGMENT_BUCKETS):
            batches = int(seg_batches[i].item())
            if batches <= 0:
                continue
            toks = seg_tokens[i].item()
            byt = seg_bytes[i].item()
            loss = seg_loss[i].item() / max(toks, 1.0)
            bpb = loss / math.log(2.0) * (toks / max(byt, 1.0))
            log(
                f"ttseg: policy:{h.ttt_tail_policy} bucket:{name} "
                f"batches:{batches} tokens:{toks:.0f} bytes:{byt:.0f} "
                f"val_loss:{loss:.8f} val_bpb:{bpb:.8f}"
            )
    for p in base_model.parameters():
        p.requires_grad_(True)
    base_model.train()
