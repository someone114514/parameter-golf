...
class Hyperparameters:
 ...
 tokenizer_path = os.environ.get("TOKENIZER_PATH", str(SCRIPT_DIR / "candidate.vocab"))
 tokenizer_meta_path = os.environ.get("TOKENIZER_META_PATH", str(SCRIPT_DIR / "candidate.meta.npz"))
 ...
 eval_mode = os.environ.get("EVAL_MODE", "sliding")
 ...
 online_ngram_enabled = bool(int(os.environ.get("ONLINE_NGRAM_ENABLED", "0")))
...
def main() -> None:
 ...
 if args.smoke_test:
  ...
  tiny = GPT(
   vocab_size=32, num_layers=2, model_dim=64, num_heads=4, num_kv_heads=2,
   mlp_mult=2.0, tie_embeddings=True, tied_embed_init_std=args.tied_embed_init_std,
   logit_softcap=args.logit_softcap, rope_base=args.rope_base, qk_gain_init=args.qk_gain_init,
   mtp_num_heads=0, mtp_loss_weight=0.0, bigram_vocab_size=64, bigram_dim=16,
   xsa_last_n=0, rope_dims=16, ln_scale=args.ln_scale, dtg=False,
   ve_enabled=False, ve_dim=args.ve_dim, ve_layers=args.ve_layers, vrl_enabled=False,
  ).bfloat16()
  for module in tiny.modules():
   if isinstance(module, CastedLinear):
    module.float()
  restore_low_dim_params_to_fp32(tiny)
  print(f"smoke_test:flash_attn_available={int(_HAS_FA3)}")
  print(f"smoke_test:model_params={sum(p.numel() for p in tiny.parameters())}")
 ...
 meta_path_env = args.tokenizer_meta_path
 ...
 final_baseline_loss = q_val_loss
 final_baseline_bpb = q_val_bpb
 if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
  ...
  final_baseline_loss = sw_val_loss
  final_baseline_bpb = sw_val_bpb
 ...
 run_ogd = args.ogd_enabled and eval_mode in {"ogd", "all"}
 run_ttt = args.ttt_enabled and eval_mode in {"ttt", "ttt_ngram", "all"}
 run_ngram = args.online_ngram_enabled and eval_mode in {"ngram", "ttt_ngram", "all"}
 if run_ogd:
  torch._dynamo.reset()
  torch.cuda.synchronize()
 ...
 if run_ngram:
  ...
  online_loss, online_bpb, online_meta = eval_val_sliding_online_best_agree(
   args=args, base_model=eval_model, rank=rank, world_size=world_size, device=device,
   ...
   stride=64, batch_seqs=args.online_ngram_batch_seqs, eval_seq_len=sw_seq_len, log0=log0,
  )
  torch.cuda.synchronize()
  log0(
   f"online_best_agree val_loss:{online_loss:.4f} val_bpb:{online_bpb:.4f} "
   f"gain_bpb:{final_baseline_bpb - online_bpb:.8f} eval_time:{1000.0 * (time.perf_counter() - t_ngram):.0f}ms"
  )
  log0(f"online_best_agree_exact val_loss:{online_loss:.8f} val_bpb:{online_bpb:.8f}")
 if distributed:
